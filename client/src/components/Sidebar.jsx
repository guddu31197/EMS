...
const Sidebar = () => {
      ...
      <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        {navItem.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolote left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500" />
              )}
              <item.icon
  ...
  return (
    <>
      ...
      {/* Sidebar - mobile */}
      <aside
        className={`lg:hidden fixed insert-y-0 left-0 w-72 bg-linear-to-b from-slate-900 to-slate-950 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebarContent}
      </aside>
    </>
